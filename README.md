...
刚开始客户端处于Closed的状态，服务器处于Listen状态。
 - 第一次握手：首先<font color="	#FF6347">客户端向服务器发送一段TCP报文</font>，其中：标记位SYN表示<font color="	#FF6347">请求建立新连接</font>；序号seq=X，X一般为1；随后客户端进入SYN-SEND阶段。
...
3. 客户端浏览器与WEB服务器建立TCP连接(三次握手)；
4. 客户端浏览器通过TCP/IP连接向对应IP地址的WEB服务器发送HTTP请求；
5. WEB服务器响应请求，返回指定的URL数据或错误信息。如果设定了重定向，则重定向到新的URL地址；
6. 客户端浏览器下载数据，解析HTML源文件，解析的过程中实现对页面的排版 ，解析完成后，在浏览器中显示基础的页面；
7. 分析页面中的超链接，显示在当前页面，重复以上过程直到没有超链接需要发送，完成页面的全部显示。
...
<font color="	#FF6347">DNS具体的查找过程</font>：
- 浏览器缓存：首先DNS会在浏览器缓存中寻找；
- 系统缓存：如果没有浏览器会进行系统调用，进而在系统缓存中寻找；
- 路由器缓存：路由器一般会有自己的DNS缓存，可以将查询请求发送向路由器；
- ISP DNS缓存：接下来就是检查ISP缓存DNS的服务器；
- 当局部DNS服务器不能查询到域名对应的IP地址时，就需要向其他DNS服务器进行查询，有两种查询方式：1. 递归搜索：一般为<font color="	#FF6347">向该域名的根域名服务器查询，再由根域名服务器一级级向下查询，最后得到的查询结果返回给局部DNS服务器，再由局部DNS服务器返回给客户端</font>。2. 迭代搜索：局部DNS服务器将能解析该域名的其他DNS服务器的IP地址返回给客户端，客户端在想这个DNS服务器进行查询。
...
### *如何防御XSS攻击
- 对<font color="	#FF6347">数据获取渲染和字符串拼接</font>的时候应该对可能出现的恶意代码情况进行判断。
...
## 进程与线程
- 进程描述了CPU在运行指令及加载和保存上下文所需的时间，放在应用上来说代表了一个程序。
- 线程是进程中的最小单位，代表了执行一段指令需要的时间。
...
### 2. 浏览器的多线程
打开一个网页，至少需要4个进程：<font color="	#FF6347">网络进程、浏览器进程、GPU进程和渲染进程，如果有插件，还需要一个插件进程</font>。

虽然多线程提升了浏览器的稳定性、流畅性和安全性，但也<font color="	#FF6347">造成了更高的资源占用和更复杂的体系架构，导致各模块之间耦合度过高，难以复用，难以适应新的需求</font>。
...
### 4. 浏览器<font color="	#FF6347">渲染进程</font>的线程有哪些
![image](https://user-images.githubusercontent.com/70066311/160823144-9939b969-30e7-4460-b752-b2c609d7595b.png)20
(1) GUI渲染进程：
负责渲染页面，解析HTML、CSS，构建DOM树。当页面发生更新或回流时，该线程就会执行。

(2) JS引擎线程：
负责处理JS脚本，一个页面同一时刻只有一个JS引擎在运行JS脚本
